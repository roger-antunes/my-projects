...
def carregar_som():
    try:
        # Tentar carregar o arquivo a partir do diretório temporário do PyInstaller
        pygame.mixer.music.load(resource_path("som.mp3"))
    except pygame.error:
        try:
            # Se não encontrar, tentar carregar o arquivo a partir da pasta atual
...
style = ttk.Style()
style.map('TButton', background=[('active', '#d9d9d9'), ('pressed', '#c9c9c9')])  # 
style.configure('TFrame', background='#f0f0f0', font=(15))  # cor de fundo padrão do sistema operacional
style.configure('TLabel', background='#f0f0f0')  # cor de fundo padrão do sistema operacional
style.configure('TButton', background='#f0f0f0')  # cor de fundo padrão do sistema operacional
...
btn_proxima = ttk.Button(btn_frame, text="Próxima Questão", command=lambda: proxima_questao())
btn_proxima.pack(side=tk.LEFT, padx=5)

btn_finalizar = ttk.Button(btn_frame, text="Finalizar e Gerar Relatório", command=finalizar_e_gerar_relatorio)
btn_finalizar.pack(side=tk.LEFT, padx=5)

btn_fechar = ttk.Button(btn_frame, text="Fechar", command=fechar_aplicacao)
